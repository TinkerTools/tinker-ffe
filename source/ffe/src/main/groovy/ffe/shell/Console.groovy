 ...
 * @author Dierk Koenig (Groovy Console)
 * @author Alan Green (Groovy Console)
 */
class Console implements CaretListener {
	...
	void run() {
		...
		frame = swing.frame(
			title:'Force Field Explorer Shell',
			location:[100,100],
			size:[500,400],
...
*/
				...
				orientation:JSplitPane.VERTICAL_SPLIT, constraints: BorderLayout.CENTER) 
			{
				...
				scrollPane {
					outputArea = textPane(editable:false, background: Color.WHITE, foreground: Color.BLACK)
					addStylesToDocument(outputArea)
				}
			}
            
			statusLabel = label(id:'status', text: 'Welcome to the Force Field Explorer Shell.', constraints: BorderLayout.SOUTH,
				border: BorderFactory.createLoweredBevelBorder())
		}   // end of frame
        
		runWaitDialog = swing.dialog(title: 'Shell executing', owner: frame, modal: true) {
			//boxLayout(axis: BoxLayout.Y_AXIS)  // todo mittie: dialog.setLayout -> dialog.contentPane.setLayout()
			label(text: "FFE is now executing. Please wait.", 
		...
		appendOutput("\n Force Field Explorer\n Version 8.6, February 2019\n" + " Copyright (c) Michael Schnieders, Jay Ponder, 2004-2019,\n All Rights Reserved\n" + " Email to: ponder@dasher.wustl.edu\n\n", commandStyle);
	}
	...
	void addStylesToDocument(JTextPane outputArea) {
		...
		StyleConstants.setBackground(resultStyle, Color.WHITE)
	}
	...
	boolean askToSaveFile() {
		...
		switch (JOptionPane.showConfirmDialog(frame,
			...
			"Force Field Explorer Shell", JOptionPane.YES_NO_CANCEL_OPTION))
		{
	...
	void confirmRunInterrupt(EventObject evt) {
		def rc = JOptionPane.showConfirmDialog(frame, "Attempt to interrupt script?", "Force Field Explorer Shell", JOptionPane.YES_NO_OPTION)
	...
	void historyNext(EventObject evt = null) {
		if (historyIndex < history.size()) {
			...
			statusLabel.text = "Can't go past end of history"
			beep()
		}
	}
	...
	void inspectLast(EventObject evt = null){
		if (null == lastResult) {
			JOptionPane.showMessageDialog(frame, "The last result is null.", "Cannot Inspect", JOptionPane.INFORMATION_MESSAGE)
			return
		}
	...
	void runScript(EventObject evt = null) {
		...
		for (line in record.textToRun.tokenize("\n")) {
			appendOutputNl('FFE > ', promptStyle)
	...
	void updateTitle() {
		if (scriptFile != null) {
			frame.title = scriptFile.name + (dirty?" * ":"") + " - Force Field Explorer Shell"
		} else {
			frame.title = "Force Field Explorer Shell"
		}
	}
}
